...
> **Gap C2** — honest semantics for `blindkey rm` and forward secrecy via `blindkey rotate-data-key`.

## What `blindkey rm` guarantees
...
For a leaked **old copy** of the vault file, run **`blindkey rotate-data-key`** after you trust the current machine again.

## `blindkey rotate-data-key`
...
- A **compromised** hardware factor was removed (`blindkey stanzas remove …`).
...
```sh
blindkey rotate-data-key --re-seal-recovery
```
