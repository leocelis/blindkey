...
to document for users: (a) `blindkey upgrade-kdf` does not re-protect old copies in backend history —
...
[`crates/blindkey-core/src/rollback/mod.rs`](../../crates/blindkey-core/src/rollback/mod.rs).
...
and a second `blindkey` process may run concurrently. Rules:
...
- `blindkey open --allow-rollback` (and the same flag on any unlocking subcommand) proceeds
