# Blindkey — Product Requirements Document (PRD)

> **Status:** Accepted v0.2 · June 2026 · **v1.0.0** (CLI + desktop app; format v1 frozen per ADR-0005)
> **Sources of truth:** [`blindkey_intent.yaml`](../blindkey_intent.yaml) (66 constraints — 60 implemented + 6 forward for UC-23, v1.8.0),
...
**Blindkey is an open-source security layer for the AI era**: a local-first credential vault for
...
4. **Existing tools are too complex to evaluate.** A developer cannot easily answer "what
   exactly does this protect me from, and how do I check?" Blindkey answers it with a public,
...
| # | Persona | Situation | What they need from Blindkey |
|---|---------|-----------|---------------------------|
...
Each use case lists the constraints (from `blindkey_intent.yaml`) that bind it. Acceptance
...
A developer installs with one command (`cargo install blindkey-cli`; static binary, no runtime
deps) and runs `blindkey init`. They choose a master password; Blindkey generates a random data
...
`blindkey gen` produces a CSPRNG password (rejection-sampled, no modulo bias) — charsets
...
On `blindkey add`, a user-supplied password below ~60 bits triggers a warning suggesting
`blindkey gen` (warn, never block).

### UC-3 · Store a secret
**Persona:** all · **Constraints:** C18, C19, C17, C11

`blindkey add NAME` stores title, username, password, URL, notes, tags, OTP secret, and
...
`blindkey get github-prod --field password`. The secret goes **to the OS clipboard, never to
stdout** — the agent's transcript of the session contains no plaintext. The clipboard
auto-clears after 30 s (configurable). Blindkey makes zero network calls, so there is no
...
`blindkey get NAME --stdout` prints the secret for piping — and prints a warning to stderr
("plaintext written to stdout; ensure no AI agent or untrusted process captures this
stream"). `blindkey export --format json` similarly requires a security warning. Plaintext
...
`blindkey ls --search`, `blindkey edit`, `blindkey rm` (with confirmation). Search runs in-memory
...
an old copy: on regression, Blindkey warns and aborts by default (exit code 2 when
...
message and a `blindkey re-enroll-tpm` path, not a lockout.

### UC-10 · Open a stale or hostile vault file safely
**Persona:** all · **Constraints:** C2 (incl. ceiling), C7, C8, C9

Blindkey treats its own file as untrusted input. Bad magic → "not a vault file". Newer format
...
`blindkey tune` benchmarks Argon2id on the current machine and recommends parameters targeting
~300 ms. `blindkey upgrade-kdf` re-derives in place. Parameters live in the file (never
...
`blindkey import --format txt|json` (with pass/gopass and KeePassXC CSV paths on the roadmap,
M6/M9) moves an existing store into Blindkey in one command. Weak imported passwords trigger
the C26 entropy warning, nudging rotation via `blindkey gen`.
...
`blindkey import --format raw keys.txt` parses it leniently, classifies secret-vs-label lines by
...
findable via `blindkey ls --search` (UC-6) and retrievable to the clipboard via `blindkey get` (UC-4) —
...
Pure-Rust **TUI** (`blindkey-tui`) and **desktop window** (`blindkey-gui`) are thin clients over `blindkey-core`
...
logged (C37), and every keystroke repaints in under 100 ms (C38). Surfaces as `blindkey find` (CLI/TUI)
...
The shipped `blindkey-gui` (egui/eframe) gets a performance and presentation-hardening pass: pin the
...
file-dialog dependencies documented. Extends UC-18 P2 without moving logic out of `blindkey-core`.
...
Fleet deployment hooks (`BLINDKEY_VAULT_PATH`, `BLINDKEY_CONFIG_DIR`, `BLINDKEY_LOCK_ON_BLUR`),
...
that needs to live on Dropbox, Drive, S3, or a git remote they don't trust. `blindkey seal <folder>`
(or drag-and-drop onto the window) produces one sealed `.vltf` blob; they upload it however they
already upload things. `blindkey open` restores it; `blindkey peek` lists the inner tree after unlock.
