...
numbered constraint (`C1`…`C31`) with a `test:` field. When you implement or change behavior:
...
for candidate areas (C32+) we already know we want.
...
- **Never accept a secret as a command-line argument** (constraint C29).
