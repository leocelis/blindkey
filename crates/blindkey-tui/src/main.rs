//! `blindkey-tui` — a fast terminal UI over `blindkey-core` (the first UC-18 shell).
//!
//! Thin shell: it unlocks the vault via `blindkey_core`, then lets you type-to-search and press Enter
...
/// UC-23 sealed-file commands (parity with `blindkey seal` / `open` / `peek`).
#[derive(Subcommand)]
enum SealedCommand {
...
mod tests {
    use super::*;
    use blindkey_core::format::entry::{Entry, Protected};
