...
fn c41_eframe_glow_pinned_no_persistence() {
    ...
    let gui_cargo = read_workspace_file("crates/blindkey-gui/Cargo.toml");
    for line in gui_cargo.lines() {
        ...
        if dep_part.starts_with("eframe") && dep_part.contains('=') {
            assert!(
                ...
                "blindkey-gui eframe dep must not enable persistence: {dep_part}"
            );
        }
    }
}
...
fn c45_thin_shell_no_direct_crypto() {
    ...
    for forbidden in [
        "chacha20poly1305",
        "argon2::",
        "blindkey_core::format::crypto",
    ] {
        assert!(
            !src.contains(forbidden),
            "blindkey-gui must not import {forbidden}"
        );
    }
