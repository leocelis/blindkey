//! `blindkey-gui` — a simple, fast, secure desktop window app over `blindkey-core` (UC-18 P2).
//!
//! A thin GUI shell: it unlocks (or creates) the vault via `blindkey_core`, then lets you
...
//!
//! Every byte that touches a secret stays inside `blindkey-core`; this binary only renders metadata,
...
fn main() -> eframe::Result<()> {
    blindkey_core::memory::harden_process(); // C25: disable core dumps before touching secrets

    let path = match gui_config::resolve_vault_path() {
        Ok(p) => p,
        Err(e) => {
            eprintln!("blindkey-gui: {e}");
            std::process::exit(1);
        }
    };
...
struct VaultApp {
    ...
    audit_report: Option<blindkey_core::audit::AuditReport>,
...
impl VaultApp {
        ...
        match opened {
            ...
            Err(e) => {
                ...
                self.error = Some(match e {
                    blindkey_core::Error::HeaderAuth => "Incorrect master password.".to_string(),
    ...
    fn try_create(&mut self) {
        ...
        if !self.allow_weak_create {
            let bits = blindkey_core::audit::password_entropy_bits(self.pw_input.as_bytes());
    ...
    fn load_import(&mut self, text: &str) {
        let r = blindkey_core::import::parse_raw(text);
    ...
    fn unlocked_screen(&mut self, ctx: &egui::Context) {
        ...
        let mut pad_on = matches!(
            ...
            blindkey_core::pad::PadMode::Padme
        );
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let otp_display: Option<(String, u64)> = e.otp_secret.as_ref().and_then(|p| {
                blindkey_core::totp::generate_now(&p.expose())
                    .ok()
                ...
                Action::Audit => {
                    if let Some(v) = self.vault.as_ref() {
                        self.audit_report = Some(blindkey_core::audit::analyze(
                            v.entries(),
                            now_unix(),
                            &blindkey_core::audit::AuditConfig::default(),
                        ));
                    }
                }
            }
    ...
    fn set_padding(&mut self, on: bool) {
        if let Some(v) = self.vault.as_mut() {
            v.set_padding(if on {
                ...
                blindkey_core::pad::PadMode::None
            });
        }
        match self.persist() {
            Ok(()) => {
    ...
    fn editor_window(&mut self, ctx: &egui::Context) {
        ...
        if generate_pass {
            if let Some(ed) = self.editor.as_mut() {
                if let Ok(p) = blindkey_core::gen::passphrase(
                    GENERATED_WORDS,
    ...
    hits.iter()
        .map(|h| {
            ...
            let positions = h
                .matches
                .iter()
                .find(|m| matches!(m.field, blindkey_core::search::Field::Title))
...
fn random_id() -> [u8; 16] {
    let mut id = [0u8; 16];
    // Same audited OS CSPRNG blindkey-core uses; on the (astronomically unlikely) failure path the id
