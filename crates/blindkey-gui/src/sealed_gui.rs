...
impl SealedGui {
    ...
    fn try_start_seal(&mut self, ctx: &egui::Context) -> bool {
        ...
        if dlg.enroll_yubikey && !blindkey_hardware::yubikey::available() {
            dlg.error = Some(
...
fn run_seal_job(
    ...
    let (final_bytes, recovery_note) = match seal_2fa {
        ...
        Seal2faAtCreate::YubiKey => {
            if let Err(e) = blindkey_hardware::yubikey::program_chalresp_slot2() {
                finish(Err(e));
                return;
            }
            ...
            let hw_response = match blindkey_hardware::yubikey::challenge_response(&challenge) {
                Ok(r) => r,
                Err(e) => {
                    finish(Err(e));
                    return;
