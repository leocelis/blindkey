...
//!
//! Priority: `--password-fd` → `--password-stdin` → `BLINDKEY_PASSWORD_FILE` → TTY prompt.
...
pub fn read_master_password(
    ...
    if let Ok(path) = std::env::var("BLINDKEY_PASSWORD_FILE") {
        if !path.is_empty() {
...
fn read_password_from_fd(fd: i32) -> Result<Zeroizing<String>, String> {
    let line = blindkey_sys::read_line_from_fd(fd)
...
fn read_password_file(path: &Path) -> Result<Zeroizing<String>, String> {
    ...
    let bytes = std::fs::read(path)
        .map_err(|e| format!("cannot read BLINDKEY_PASSWORD_FILE {}: {e}", path.display()))?;
    if bytes.is_empty() {
        return Err(auth_err("BLINDKEY_PASSWORD_FILE is empty"));
    }
    let line = bytes
        .split(|&b| b == b'\n' || b == b'\r')
        .next()
    ...
    Ok(Zeroizing::new(
        std::str::from_utf8(line)
            .map_err(|_| "BLINDKEY_PASSWORD_FILE must be valid UTF-8".to_string())?
            .to_string(),
    ))
}
...
fn check_mode_0600(path: &Path) -> Result<(), String> {
    ...
    let meta = std::fs::metadata(path)
        .map_err(|e| format!("cannot stat BLINDKEY_PASSWORD_FILE {}: {e}", path.display()))?;
