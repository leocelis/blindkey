//! Agent CLI — `blindkey agent …` (S-13 scaffold).

use std::path::{Path, PathBuf};

use blindkey_agent::paths;
use blindkey_agent::{
...
fn cmd_allow(
    ...
    let cmd =
        for_cmd.ok_or("usage: blindkey agent allow NAME --dest-env VAR --for-cmd /path/to/cmd")?;
    ...
    eprintln!("Start broker: blindkey agent run");
    Ok(())
}
