...
const PATTERN_NEEDLES: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &["blindkey seal", "blindkey open", "blindkey peek"],
    ),
...
fn intent_v180_forward_constraints_and_sc9() {
    let intent = read_repo_file("blindkey_intent.yaml");
    assert!(
...
fn uc23_spec_matches_intent_and_patterns() {
    ...
    assert!(spec.contains("Desktop app (`blindkey-gui`) design"));
