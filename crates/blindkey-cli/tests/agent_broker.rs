...
fn agent_broker_research_and_adr_exist() {
    ...
    assert!(read_repo_file("docs/adr/0006-agent-broker-scaffold.md").contains("blindkey-agent"));
}

#[test]
fn agent_broker_guide_exists() {
    let guide = read_repo_file("docs/AGENT_BROKER.md");
    assert!(guide.contains("blindkey agent allow"));
    assert!(guide.contains("status only"));
}

#[test]
fn cli_documents_blindkey_agent() {
    let cli = read_repo_file("docs/CLI.md");
    assert!(cli.contains("blindkey agent"));
}

#[test]
fn use_response_never_includes_password_key() {
    let json = blindkey_agent::UseResponse::ok().to_json_line().unwrap();
