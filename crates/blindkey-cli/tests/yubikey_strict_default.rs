...
fn core_exports_yubikey_stale_warning() {
    assert!(blindkey_core::YUBIKEY_STALE_WARNING.contains("not refreshed"));
}

#[test]
fn payload_yubikey_strict_tlv_round_trips() {
    use blindkey_core::format::entry::Protected;
    use blindkey_core::format::payload::{Payload, INNER_STREAM_KEY_LEN};
    ...
    let p = Payload {
        ...
        yubikey_strict: true,
        entries: vec![],
        usage: blindkey_core::frecency::FrecencyStore::new(),
    };
