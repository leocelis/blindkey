...
#[test]
fn blindkey_core_exposes_rotate_data_key() {
    let lib = read_repo_file("crates/blindkey-core/src/vault.rs");
    assert!(
...
fn stanzas_remove_mentions_rotate_hint() {
    let cmd = read_repo_file("crates/blindkey-cli/src/commands/mod.rs");
    assert!(
