//! End-to-end CLI integration test: drives the built `blindkey` binary against the sample `keys.txt`
...
/// Run the `blindkey` binary under an isolated `home`, feeding `stdin`. Returns (exit code, out, err).
...
fn vault_password_file_env_unlocks() {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_blindkey"))
        .env("HOME", &home)
        .env("XDG_DATA_HOME", home.join("share"))
...
fn c27_headless_get_exits_7_without_stdout() {
    ...
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_blindkey"));
    cmd.env("HOME", &home)
        .env("XDG_DATA_HOME", home.join("share"))
        .env_remove("DISPLAY")
