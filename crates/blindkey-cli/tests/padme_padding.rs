...
fn padme_guide_documents_default_off_and_toggle() {
    ...
    for needle in ["Default: off", "blindkey pad on", "Padmé", "mtime"] {
        assert!(
...
fn pad_rs_defaults_none_and_exports_padme() {
    let pad = read_repo_file("crates/blindkey-core/src/pad.rs");
...
fn cli_documents_pad_command() {
    ...
    assert!(cli.contains("blindkey pad") && cli.contains("Padmé"));
}
