...
#[test]
fn blindkey_sys_documents_ptrace_and_coredump_filter() {
    let lib = read_repo_file("crates/blindkey-sys/src/lib.rs");
    for needle in ["PR_SET_DUMPABLE", "coredump_filter", "ptrace"] {
        assert!(lib.contains(needle), "blindkey-sys missing: {needle}");
    }
}
...
fn mains_call_harden_process_before_secrets() {
    for rel in [
        "crates/blindkey-cli/src/main.rs",
        "crates/blindkey-tui/src/main.rs",
        "crates/blindkey-gui/src/main.rs",
    ] {
