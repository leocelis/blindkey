...
fn c23_no_network_dependencies_in_shipped_crates() {
    for rel in [
        "crates/blindkey-cli/Cargo.toml",
        "crates/blindkey-core/Cargo.toml",
        "crates/blindkey-gui/Cargo.toml",
        "crates/blindkey-tui/Cargo.toml",
    ] {
    ...
    let main_rs =
        std::fs::read_to_string(repo_root().join("crates/blindkey-cli/src/main.rs")).unwrap();
...
fn c57_cli_honors_vault_vault_path_env() {
    let src = std::fs::read_to_string(repo_root().join("crates/blindkey-cli/src/commands/mod.rs"))
        .unwrap();
    assert!(src.contains("BLINDKEY_VAULT_PATH"));
}
...
fn c31_unlock_secret_channels_wired() {
    let main_rs =
        std::fs::read_to_string(repo_root().join("crates/blindkey-cli/src/main.rs")).unwrap();
    let unlock =
        std::fs::read_to_string(repo_root().join("crates/blindkey-cli/src/unlock_secret.rs"))
            .unwrap();
    assert!(main_rs.contains("password_fd"));
    assert!(main_rs.contains("password_stdin"));
    assert!(unlock.contains("BLINDKEY_PASSWORD_FILE"));
