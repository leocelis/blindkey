...
use blindkey_core::crypto::kdf;
use blindkey_core::envelope::{generate_data_key, unwrap_password_stanza, wrap_password_stanza};
use blindkey_core::format::{Entry, Protected};
...
fn c3_audited_crypto_deps_and_deny_policy() {
    let core_toml =
        std::fs::read_to_string(repo_root().join("crates/blindkey-core/Cargo.toml")).unwrap();
    for dep in [
        "chacha20poly1305",
        "argon2",
        "hkdf",
        ...
        assert!(
            core_toml.contains(dep),
            "blindkey-core must use audited {dep}"
        );
    }
    assert!(
        !core_toml.contains("openssl"),
        "openssl must not appear in blindkey-core deps (C3)"
    );
    ...
    let crypto_mod =
        std::fs::read_to_string(repo_root().join("crates/blindkey-core/src/crypto/mod.rs"))
            .unwrap();
    assert!(
