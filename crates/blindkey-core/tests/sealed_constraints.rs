...
use blindkey_core::format::file_archive::validate_inner_path;
...
fn c64_body_corruption_matrix_uniform_error() {
    ...
    let blob = container
        ...
        .unwrap();
    let header = blindkey_core::format::Header::parse_with_kind(
        &blob,
        Some(blindkey_core::ContainerKind::SealedFile),
    )
    .unwrap();
...
fn c63_sealed_throughput_release_bench() {
    ...
    let floor: f64 = std::env::var("BLINDKEY_SEAL_BENCH_MIN_MIB_S")
        .ok()
    ...
    assert!(
        ...
        "A23: seal={seal_mib_s:.1} MiB/s open={open_mib_s:.1} MiB/s (floor {floor}; set \
         BLINDKEY_SEAL_BENCH_MIN_MIB_S=400 on reference hardware per UC-23 §3.5)"
    );
    let _ = fs::remove_dir_all(&base);
}
