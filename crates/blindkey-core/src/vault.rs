//! The vault open/save orchestration — ties every layer together (the v0 `blindkey-core` API).
//!
...
/// key's HMAC-SHA1 response (the physical-tap step). Lives behind a `dyn` so `blindkey-core` never
/// depends on the USB layer (the CLI/GUI supply the closure; `blindkey-hardware` does the I/O).
