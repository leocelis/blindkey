...
pub fn unwrap_fido2_stanza(
    ...
    if extra.salt_hash != blindkey_hardware_salt(vault_id) {
        return Err(Error::HeaderAuth);
    }
...
/// C6 salt recipe (duplicated here so blindkey-core tests do not depend on blindkey-hardware).
pub fn blindkey_hardware_salt(vault_id: &[u8; 16]) -> [u8; 32] {
...
mod tests {
    ...
    fn fido2_stanza_wrap_unwrap() {
        ...
        let extra = Fido2Extra {
            ...
            relying_party_id: "vault.local".into(),
            salt_hash: blindkey_hardware_salt(&vid),
        };
