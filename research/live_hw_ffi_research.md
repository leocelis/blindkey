...
| Question | Answer |
|----------|--------|
| Rust `libfido2-sys` / `tss-esapi` in-tree? | **Deferred** — `blindkey-sys` stays OS-hardening-only; new deps need vet + CI images |
...
- Salt: `SHA-256(vault_id ‖ "fido2-hw-v1")` — [`fido2_salt.rs`](../crates/blindkey-hardware/src/fido2_salt.rs).
...
- PCR mismatch → verbatim C15 message + `blindkey re-enroll-tpm`.
